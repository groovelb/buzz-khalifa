...
export const Building: React.FC = () => {
  ...
  useFrame(() => {
    ...
    if (groupRef.current) {
      ...
      if (offset < SCENE_TIMING.constructionEnd) {
        // 건설 단계: 계속 아래로 스크롤
        // Multiplier 55 aligns tower top (47 units) with scroll 0.857
        targetY = -offset * SCENE_TIMING.buildingTravel;
      } else {
        // 최종 단계: 스파이어 + 전체 건물 reveal
        // 건물 중앙 = 전체 높이(55)의 절반 = 27.5
        const finalProgress = (offset - SCENE_TIMING.constructionEnd) / SCENE_TIMING.finaleLength;
